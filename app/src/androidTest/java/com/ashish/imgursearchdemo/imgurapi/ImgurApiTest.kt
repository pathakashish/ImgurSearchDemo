...
class ImgurApiTest {
    ...
    fun testThatRetrofitProvidesCorrectSearchResultObjectGivenRightJsonData() {
        ...
        val expectedSearchResult = getExpectedSearchResult()
        val apiService = getImgurApi(mockWebServer)
        val searchResult =
            apiService.search(0, "test").blockingFirst()

        assertEquals(expectedSearchResult, searchResult)
        assertTrue(searchResult.data.contains(getDataWithGif()))
        assertTrue(searchResult.data.contains(getDataWithNonGif()))
        assertTrue(searchResult.data.contains(getDataWithStaticJpeg()))
        assertTrue(searchResult.data.contains(getDataWithImages()))

        mockWebServer.shutdown()
    }

    private fun getExpectedSearchResult(): SearchResult {
        return SearchResult(
            data = listOf(
                getDataWithImages(),
                getDataWithGif(),
    ...
    private fun getDataWithNonGif() = Data(
        id = "aYmcqEH",
    ...
    private fun getDataWithGif() = Data(
        id = "lEBF1kl",
        title = "51 years ago...",
        type = "image/gif",
        animated = true,
    ...
    private fun getDataWithStaticJpeg() = Data(
        id = "rfV0ljM",
    ...
    private fun getDataWithImages() = Data(
        id = "78mBcjV",
    ...
    private fun getVideoImage() = Image(
        id = "aWj3AsE",
        title = null,
        type = "video/mp4",
        animated = true,
    ...
    private fun getStaticJpegImage() = Image(
        id = "6MU96OI",
        title = null,
        type = "image/jpeg",
        animated = false,
    ...
    private fun getImgurApi(mockWebServer: MockWebServer) = ImgurApiModule.provideImgurApi(
